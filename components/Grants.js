export const grants = [
  {
    id: "0xcde649b95fd2d6bbfbe036c9e74188a5302bda0dfa48730333b9f9b825a82924",
    anchor: "0x2bd2E7777b2581E5cb7B6fE4aAdeE2eeA8ECa038",
    githubLink: "https://github.com/airaffairdrones",
    twitterLink: "https://twitter.com/airaffairdrones",
    name: "AirAffair Drones",
    createdAt: "2021-07-26",
    description:
      "AirAffair - A drone service that plants seeds in deforested areas. Rebuilding our forests from the sky.",
    websiteUrl: "https://airaffairdrones.org",
    logoImg: "https://i.ibb.co/HpnPZdV/airaffair-drones-logo.png",
    bannerImg: "https://i.ibb.co/71fhcnJ/airaffair-drones.png",
  },
  {
    id: "0xdc9b7349f52b60250399623245a1879367e9b156dfd460ce2a315af1e7cc7ebc",
    anchor: "0x029b6A142E047A19be1626688a1cC3d075F4a5af",
    githubLink: "https://github.com/ecodriveev",
    twitterLink: "https://twitter.com/ecodriveev",
    name: "EcoDrive EV",
    createdAt: "2021-02-10",
    description:
      "EcoDrive - Electric vehicles that are charged solely through renewable energy sources. Making commutes green and clean.",
    websiteUrl: "https://ecodriveev.org",
    logoImg: "https://i.ibb.co/xDWV0yy/ecodrive-ev-logo.png",
    bannerImg: "https://i.ibb.co/HXBb9PT/ecodrive-ev.png",
  },
  {
    id: "0xdd24c7e8a35d4a95f0f4b8f5c819b7c2a0029807a39b0a2f8cb4bf51141816ff",
    anchor: "0xbAec44f49Df3578749Fb299769D723eAccDdB605",
    githubLink: "https://github.com/waterwebai",
    twitterLink: "https://twitter.com/waterwebai",
    name: "WaterWeb AI",
    createdAt: "2020-02-23",
    description:
      "WaterWeb - A smart irrigation system that uses AI to optimize water usage in farming. Saving water, one drop at a time.",
    websiteUrl: "https://waterwebai.org",
    logoImg: "https://i.ibb.co/1vZwC89/waterweb-ai-logo.png",
    bannerImg: "https://i.ibb.co/VDXtBnC/waterweb-ai.png",
  },
  {
    id: "0xe95e5d6d22ca3926fea364e04d1a47faf8abedadc60a1409d0d28c92462ce586",
    anchor: "0x4501285E8FFE2d38828F1E69FB57a6a2fAAC2620",
    githubLink: "https://github.com/naturtokencrypto",
    twitterLink: "https://twitter.com/naturtokencrypto",
    name: "NaturToken Crypto",
    createdAt: "2022-10-10",
    description:
      "NaturToken - A cryptocurrency that plants a tree for every transaction. Making the digital world greener with each click.",
    websiteUrl: "https://naturtokencrypto.org",
    logoImg: "https://i.ibb.co/qDrJns8/naturtoken-crypto-logo.png",
    bannerImg: "https://i.ibb.co/5j2WYwY/naturtoken-crypto.png",
  },
  {
    id: "0xec3b888b11acdf6cab7a35c97b2435487c93f28e86e7ad456483dd46ba5f3dae",
    anchor: "0xa255AE8258627f211883f575d0BBcC9AA67A74E8",
    githubLink: "https://github.com/sunburstgrid",
    twitterLink: "https://twitter.com/sunburstgrid",
    name: "SunBurst Grid",
    createdAt: "2023-04-10",
    description:
      "SunBurst - A decentralized energy grid where individuals can share excess solar energy with their community. Powering our future, one neighborhood at a time.",
    websiteUrl: "https://sunburstgrid.org",
    logoImg: "https://i.ibb.co/bN8bcJ4/sunburst-grid-logo.png",
    bannerImg: "https://i.ibb.co/vZZdK6h/sunburst-grid.png",
  },
  {
    id: "0xee8cde89dc8ebeaff54aaea3a59c79f49dfacd43ac349d532b7b7abbd598639f",
    anchor: "0x8Ce6D035a518205d08eb6245D11e6bE021209B4a",
    githubLink: "https://github.com/greenprint3d",
    twitterLink: "https://twitter.com/greenprint3d",
    name: "GreenPrint 3D",
    createdAt: "2020-05-12",
    description:
      "GreenPrint - A 3D printing service that uses recycled materials. We aim to reduce waste in manufacturing and promote circular economy practices.",
    websiteUrl: "https://greenprint3d.org",
    logoImg: "https://i.ibb.co/6s2d8zP/greenprint-3d-logo.png",
    bannerImg: "https://i.ibb.co/3Y60Tv6/greenprint-3d.png",
  },
  {
    id: "0xef9877949861c6eac0d3981c54834e8be9caed59816c68fd69fdfe3ee5cae4e3",
    anchor: "0xf1aF66123f87488e3176bE039e44B0d084460490",
    githubLink: "https://github.com/solarwavecomm",
    twitterLink: "https://twitter.com/solarwavecomm",
    name: "SolarWave Comm",
    createdAt: "2020-02-20",
    description:
      "SolarWave - Harnessing the power of the sun to create decentralized communication tools. Walkie-talkies, radios, and communication apps that run solely on solar energy.",
    websiteUrl: "https://solarwavecomm.org",
    logoImg: "https://i.ibb.co/kMgk7Pg/solarwave-comm-logo.png",
    bannerImg: "https://i.ibb.co/0c4XcVr/solarwave-comm.png",
  },
  {
    id: "0xf0e7cb0e1455c389165fe3eb251c994bda127924843a1178504fcd54bb069a60",
    anchor: "0x69646Cc8e833A808Edbb1ee8def5B0BBde3879ee",
    githubLink: "https://github.com/econetservices",
    twitterLink: "https://twitter.com/econetservices",
    name: "EcoNet Services",
    createdAt: "2023-03-14",
    description:
      "EcoNet - A decentralized internet service that utilizes renewable energy sources. Our vision is to create a sustainable web, free from the clutches of power-intensive servers.",
    websiteUrl: "https://econetservices.org",
    logoImg: "https://i.ibb.co/7VffMP8/econet-services-logo.png",
    bannerImg: "https://i.ibb.co/K9pGZZR/econet-services.png",
  },
  {
    id: "0xf2922c25db6ec4f23272300ef5fd06b1de4fe09567b98c54625f7162329c984b",
    anchor: "0x5cF03dEcCD84B5Ec7e51aeC3706bc7aA24dc8dEA",
    githubLink: "https://github.com/furoacompute",
    twitterLink: "https://twitter.com/furoacompute",
    name: "Furoa Compute",
    createdAt: "2022-04-02",
    description:
      "Furoa is a symbiosis of two products: a supercomputer disguised as a heating system and a decentralized cloud compute platform. It's built for high-performance compute tasks like AI training/inference, rendering CGI, performing simulations, processing audio/video data, or running crypto staking nodes.",
    websiteUrl: "https://furoacompute.org",
    logoImg: "https://i.ibb.co/9gDxHn5/furoa-compute-logo.png",
    bannerImg: "https://i.ibb.co/njJS9bw/furoa-compute.png",
  },
  {
    id: "0xf63bbcfe56807e4a1f97aff8571dd4477c91546d1fb9dbc1b2d8bc09f1a465d7",
    anchor: "0x925f808FD3C71E47D2D1a133d971d7fb74132cA2",
    githubLink: "https://github.com/earthcoininitiative",
    twitterLink: "https://twitter.com/earthcoininitiative",
    name: "EarthCoin Initiative",
    createdAt: "2022-02-01",
    description:
      "Hey Solarpunk, stoked to have you here 🌞\n" +
      "\n" +
      "Our mission - Put Eco in Economy\n" +
      "\n" +
      "Our Vision - Seed the Solarpunk paradigm\n" +
      "\n" +
      "To make this happen we are building $Earth - A digital currency that's backed by real-world climate assets, ecosystem services provided by those assets, and solarpunk values.",
    websiteUrl: "https://earthcoininitiative.org",
    logoImg: "https://i.ibb.co/42V1xhP/earthcoin-initiative-logo.png",
    bannerImg: "https://i.ibb.co/KhkZ5Jp/earthcoin-initiative.png",
  },
];
